...
def scrape_promoter_ceo_and_project_details(url):
    ...
    has_next_page = True  # Flag to track if we have a next page
        ...
        while has_next_page:
            ...
            for index in range(total_buttons):
                try:
                    ...
                    for modal_row in modal_rows:
                        try:
                            ...
                            for i in range(0, len(cols), 2):
                                ...
                                # Populate data dictionary based on labels
                                if 'promoter name' in label:
                                    data["Promoter Name"] = value
                                elif 'registration number' in label:
                                    data["Registration Number"] = value
                                elif 'pan' in label and not value_elem.find_elements(By.TAG_NAME, 'a'):  # Ensure it's not a link (PDF)
                                    data["PAN"] = value
                                elif 'gstin' in label:
                                    data["GSTIN"] = value
                    ...
                    # Switch to 'menu2' (Project Details) tab
                    ...
                    for project_row in project_modal_rows:
                        try:
                            ...
                            for i in range(0, len(cols), 2):
                                ...
                                # Populate project data based on labels
                                ...
                                elif 'total covered area' in label:
                    ...
                    # Switch to 'Bank Details' tab (menu4)
                    ...
                    for bank_row in bank_modal_rows:
                        try:
                            ...
                            for i in range(0, len(cols), 2):
                                ...
                                # Populate bank data based on labels
                    ...
                    close_button = driver.find_element(By.CSS_SELECTOR, 'button.btn-danger')
                    click_with_retry(driver, close_button)

                    print("Data scraped and modal closed successfully.\n")

                except Exception as e:
                    print(f"Error scraping data for button {index + 1}: {str(e)}")
                    continue
            ...
            try:
                ...
                if 'disabled' in next_button.get_attribute('class'):
                    ...
                    click_with_retry(driver, next_button)  # Click the next button to go to the next page
                    WebDriverWait(driver, 10).until(
                        EC.visibility_of_element_located((By.CSS_SELECTOR, '.dataTables_scrollBody tbody'))
                    )
    ...
    keys = all_data[0].keys()  # Get the column names from the first row of data
...
# Usage
scrape_promoter_ceo_and_project_details("https://rera.karnataka.gov.in/viewAllProjects")
