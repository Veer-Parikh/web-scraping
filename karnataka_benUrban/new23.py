    ...
    try:
        ...
        for index in range(total_buttons):
            try:
                ...
                print(f"Timeout while processing button {index + 1}. Skipping...")
            except StaleElementReferenceException:
                print(f"Stale element detected on button {index + 1}. Retrying...")
            except Exception as e:
                print(f"Error while processing button {index + 1}: {str(e)}")

    finally:
        # Close the driver after scraping
        driver.quit()

    # Write data to a CSV file if any data was collected
    if all_data:
        keys = all_data[0].keys()  # Get the headers from the first dictionary
        with open('rera_data.csv', 'w', newline='', encoding='utf-8') as csvfile:
