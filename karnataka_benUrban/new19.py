    ...
    try:
        ...
        for index in range(total_buttons):
            try:
                ...
                # Add the data to all_data if at least one field is filled
                if any(data.values()):
        ...
        with open('promoter_and_ceo_bank_details.csv', 'w', newline='', encoding='utf-8') as file:
            fieldnames = ["Promoter Name", "Registration Number", "PAN", "GSTIN", "District", "Taluk", "CEO/MD Name", "Designation", "DIN", "Account Name", "Bank Name"]
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
            for data in all_data:
                writer.writerow(data)

        print("\nData scraped and saved to promoter_and_ceo_bank_details.csv")
