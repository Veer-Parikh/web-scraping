...
def navigate_to_page(page_number):
    # Navigate to the homepage and reselect "Jaipur"
    ...
    # Click "Next" until the target page number is reached
    current_page = 1
    while current_page < page_number:
        next_button = driver.find_element(By.XPATH, f'//a[@data-p="{current_page + 1}" and contains(@class, "ds4u-pager-btn")]')
        next_button.click()
...
try:
    ...
    count = 1  # Initialize page count
    while True:
        # Step 1: Navigate to the page corresponding to 'count'
        navigate_to_page(count)
        scrape_page()
        
        # Increase the count after scraping each page
        count += 1

        # Optional: You can set a limit for the number of iterations if you want to stop at a certain point
        if count > 100:  # Example limit (adjust based on your need)
            break
